...
class HistAdminFormation extends StatefulWidget {
  final int idFormation;
  const HistAdminFormation({Key? key, required this.idFormation})
      : super(key: key);

  @override
  _HistAdminFormationState createState() => _HistAdminFormationState();
}

class _HistAdminFormationState extends State<HistAdminFormation> {
  ...
  getHist() async {
    ...
    var url = "$serverDir/GET_HIST_ADMIN_FORMATION.php";
    print("url=$url");
    var body = {};
    ...
    return SafeArea(
        child: Scaffold(
            ...
            appBar: AppBar(
                ...
                centerTitle: true,
                titleSpacing: 0,
                title: const Center(child: Text("Historique d'administration")),
  ...
  String constructTitle(int i) {
    return myHist[i].adminName +
        (myHist[i].action == 2
            ? " à donner les droits d'administration de cette formation au candidat "
            : " à annulé les droits d'administration de cette formation au candidat ") +
        myHist[i].userName;
  }

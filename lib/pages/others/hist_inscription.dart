...
class HistInsciption extends StatefulWidget {
  final int idFormation;
  const HistInsciption({Key? key, required this.idFormation}) : super(key: key);

  @override
  _HistInsciptionState createState() => _HistInsciptionState();
}

class _HistInsciptionState extends State<HistInsciption> {
  ...
  getHist() async {
    ...
    var url = "$serverDir/GET_HIST_ACCESS.php";
    print("url=$url");
    var body = {};
    body['ID_FORMATION'] = idFormation.toString(); body['ID_USER'] = Data.currentUser!.idUser.toString();
    ...
    return SafeArea(
        child: Scaffold(
            ...
            appBar: AppBar(
                ...
                centerTitle: true,
                titleSpacing: 0,
                title: const Center(child: Text("Historique d'Inscription")),
  ...
  String constructTitle(int i) {
    return myHist[i].adminName +
        (myHist[i].action == 1
            ? " à inscris le candidiat "
            : " à annulé l'inscription du candidat ") +
        myHist[i].userName;
  }
